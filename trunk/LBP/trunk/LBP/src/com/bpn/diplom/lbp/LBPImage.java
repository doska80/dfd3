...
public class LBPImage {
	...
	public static final Point [] AROUND_16_POINTS = new  java.awt.Point[]{
		new  java.awt.Point(-3, 1),
		new  java.awt.Point(-2, 2),
		new  java.awt.Point(-1, 3),
		new  java.awt.Point( 0, 3),
		new  java.awt.Point( 1, 3),
		new  java.awt.Point( 2, 2),
		new  java.awt.Point( 3, 1),
		new  java.awt.Point( 3, 0),
		new  java.awt.Point( 3,-1),
		new  java.awt.Point( 2,-2),
		new  java.awt.Point( 1,-3),
		new  java.awt.Point( 0,-3),
...
//	public static final int [] PRIORITY_7_BLOCKS = new  int[]{
//		0, 0, 0, 0, 0, 0, 0,
//		0, 4, 4, 3, 4, 4, 0,
//		0, 1, 1, 2, 1, 1, 0,
//		0, 1, 1, 2, 1, 1, 0,
...
//		System.out.println(img2.getFaceVector()[10]);
	...
	/**
	 * Из файла читает кратинку и зразу строит вектор изображения
	 * @param pathImg
	 * @param around
	 * @param countBlock
...
//		this.printImageBlocks(blocks);
	...
	/**
	 * Поготавливает обьект для построения вектора лица по указанным характеристикам. Для генерации вектора использовать метод getFaceVectorByRaster 
	 * @param around
	 * @param countBlock
	 */
	public LBPImage(Point [] around, int countBlock) {
		this.aroundPoints = around;
		this.radiusX = 0;
		this.radiusY = 0;
		for(Point p : aroundPoints){
	...
	public int[] getFaceVectorByRaster(Raster raster){
		System.out.println(raster.getMinX()+" "+raster.getMinY()+" "+raster.getWidth()+" "+raster.getHeight());
		this.raster = raster;
		this.widthRaster = raster.getWidth();
		this.heigthRaster = raster.getHeight();
		this.imageMatrix = this.buildImageMatrix(raster);
...
//		int toY = heigth - (heigth - heigthBlock*countBlock - fromY);
		blocks = new Block[countBlock * countBlock];
		System.out.println("New arr blocks : "+countBlock * countBlock);
		int index = 0;
//		System.out.println("Разбиваем лицо на " + countBlock+"*"+ countBlock + " блоков. Размер блока: " + widthBlock+"*"+heigthBlock);
//		System.out.println("Позиционирование блоков: x:"+fromX+"-"+toX+" y:"+fromY+"-"+toY);
		for(int y = 0; y < countBlock; y++){
...
//			System.out.println(x+" "+y);
	...
	private Set<Integer> getUniformPatternSet(int countBit){
		...
		for(int i = 0; i < countBit - 2; i++){
			...
			if(LBPUtil.isLDPCode(stNum0) && LBPUtil.isLDPCode(stNum1)){
				set.add(stNum0);
				set.add(stNum1);
			}else
				return null;
		...
		for(String pattern : set){
			Integer intPattern = LBPUtil.binToInt(pattern);
			if(intPattern >= 0){
	...
	public synchronized Set<Integer> getSetUniformPattern() {
		return setUniformPattern;
	}
